...
def PPM(data, weight_map, input, in_channels, out_channels, stride, layer_name):
    # data 尺度是 [1 2048 32 32]
    # 输出的pool尺度是pool (1, 2, 3, 6)
    G_Average_pool = network.add_pooling(input=data.get_output(0),
                                window_size=trt.DimsHW(32, 32),
                                type=trt.PoolingType.AVERAGE)
    assert G_Average_pool
    G_Average_pool.stride = (32, 32)


    Average_pool1 = network.add_pooling(input=data.get_output(0),
                                window_size=trt.DimsHW(16, 16),
                                type=trt.PoolingType.AVERAGE)
    assert Average_pool1
    Average_pool1.stride = (16, 16)
    Average_pool1.padding = (1, 1)



    Average_pool2 = network.add_pooling(input=data.get_output(0),
                                window_size=trt.DimsHW(12, 12),
                                type=trt.PoolingType.AVERAGE)
    assert Average_pool2
    Average_pool2.stride = (10, 10)
    Average_pool2.padding = (1, 1)



    Average_pool3 = network.add_pooling(input=data.get_output(0),
                                window_size=trt.DimsHW(7, 7),
                                type=trt.PoolingType.AVERAGE)
    assert Average_pool3
    Average_pool3.stride = (5, 5)
    Average_pool3.padding = (1, 1)


    conv0 = network.add_convolution(input=G_Average_pool.get_output(0),
                                    num_output_maps=512,
                                    kernel_shape=(1, 1),
                                    kernel=weight_map["decode_head.psp_modules.0.1.conv.weight"],
                                    bias=trt.Weights())
    assert conv0

    bn0 = addBatchNorm2d(network, weight_map, conv0.get_output(0),
                         "decode_head.psp_modules.0.1.bn", EPS)
    assert bn0
    ...
    conv1 = network.add_convolution(input=Average_pool1.get_output(0),
                                    num_output_maps=512,
                                    kernel_shape=(1, 1),
                                    kernel=weight_map["decode_head.psp_modules.1.1.conv.weight"],
                                    bias=trt.Weights())
    assert conv1
    bn1 = addBatchNorm2d(network, weight_map, conv1.get_output(0),
                         "decode_head.psp_modules.1.1.bn", EPS)
    assert bn1
    ...
    conv2 = network.add_convolution(input=Average_pool2.get_output(0),
                                    num_output_maps=512,
                                    kernel_shape=(1, 1),
                                    kernel=weight_map["decode_head.psp_modules.2.1.conv.weight"],
                                    bias=trt.Weights())
    assert conv2
    bn2 = addBatchNorm2d(network, weight_map, conv2.get_output(0),
                         "decode_head.psp_modules.2.1.bn", EPS)
    assert bn2
    ...
    conv3 = network.add_convolution(input=Average_pool3.get_output(0),
                                    num_output_maps=512,
                                    kernel_shape=(1, 1),
                                    kernel=weight_map["decode_head.psp_modules.3.1.conv.weight"],
                                    bias=trt.Weights())
    assert conv3
    bn3 = addBatchNorm2d(network, weight_map, conv3.get_output(0),
                         "decode_head.psp_modules.3.1.bn", EPS)
    assert bn3
    relu3 = network.add_activation(bn3.get_output(0),
                                   type=trt.ActivationType.RELU)
    assert relu3
    return  relu0,relu1,relu2,relu3
